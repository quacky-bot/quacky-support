...
class Admin(commands.Cog):
    ...
    async def sdemote(self, ctx, user, *, reason):
        ...
        demotion_embed = discord.Embed(color=discord.Colour(0xC70039), title="Woah! Imagine getting demoted", timestamp=datetime.datetime.utcnow())
        demotion_embed.description = f"Yo **{user.display_name}** how did you fuck up so hard to get demoted to: "
        warn = ''
        if mod in user.roles:
            ...
            demotion_embed.description += new_rank
            await staff_chat.send(embed=demotion_embed)
            ...
            try:
                await user.send(embed=embed)
                demotion_embed.description += "Normal User"
                await staff_chat.send(embed=demotion_embed)
    ...
    async def spromote(self, ctx, *, user):
        ...
        staff = ctx.guild.get_role(665423057430511626)
        staff_chat = ctx.guild.get_channel(665426967692181514)
        promotion_embed = discord.Embed(color=discord.Colour(7506394), title=f"Hm? What's that? A promotion?!", timestamp=datetime.datetime.utcnow())
        promotion_embed.description = f"Eyo! Let's congratulate **{user.display_time}** cause they got promoted to: "
        warn = ''
        if mod in user.roles:
            return await ctx.send(f'<:redx:678014058590502912> I cannot promote {user.display_name} as they are already the highest rank!')
        elif support in user.roles:
            new_rank = 'Mod'
            promotion_embed.description += new_rank
            await staff_chat.send(embed=promotion_embed)
